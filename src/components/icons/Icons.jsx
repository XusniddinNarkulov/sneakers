export const HeartOutlined = () => (
   <svg
      width="24"
      height="22"
      viewBox="0 0 24 22"
      fill="none"
      xmlns="http://www.w3.org/2000/svg"
   >
      <path
         d="M20.84 3.60987C20.3292 3.09888 19.7228 2.69352 19.0554 2.41696C18.3879 2.14039 17.6725 1.99805 16.95 1.99805C16.2275 1.99805 15.5121 2.14039 14.8446 2.41696C14.1772 2.69352 13.5708 3.09888 13.06 3.60987L12 4.66987L10.94 3.60987C9.9083 2.57818 8.50903 1.99858 7.05 1.99858C5.59096 1.99858 4.19169 2.57818 3.16 3.60987C2.1283 4.64156 1.54871 6.04084 1.54871 7.49987C1.54871 8.95891 2.1283 10.3582 3.16 11.3899L4.22 12.4499L12 20.2299L19.78 12.4499L20.84 11.3899C21.351 10.8791 21.7563 10.2727 22.0329 9.60523C22.3095 8.93777 22.4518 8.22236 22.4518 7.49987C22.4518 6.77738 22.3095 6.06198 22.0329 5.39452C21.7563 4.72706 21.351 4.12063 20.84 3.60987V3.60987Z"
         stroke="#FF3C78"
         stroke-width="2"
         stroke-linecap="round"
         stroke-linejoin="round"
      />
   </svg>
);

export const CheckCircleIcon = () => (
   <svg
      width="28"
      height="28"
      viewBox="0 0 28 28"
      fill="none"
      xmlns="http://www.w3.org/2000/svg"
   >
      <circle cx="14" cy="14" r="14" fill="#02BE83" />
      <path
         d="M21.6666 9L12.5 18.1667L8.33331 14"
         stroke="white"
         stroke-width="2"
         stroke-linecap="round"
         stroke-linejoin="round"
      />
   </svg>
);

export const LockIcon = () => (
   <svg
      width="16"
      height="16"
      viewBox="0 0 16 16"
      fill="none"
      xmlns="http://www.w3.org/2000/svg"
   >
      <path
         d="M12.6667 7.33325H3.33333C2.59695 7.33325 2 7.93021 2 8.66659V13.3333C2 14.0696 2.59695 14.6666 3.33333 14.6666H12.6667C13.403 14.6666 14 14.0696 14 13.3333V8.66659C14 7.93021 13.403 7.33325 12.6667 7.33325Z"
         stroke="#315BFF"
         stroke-width="2"
         stroke-linecap="round"
         stroke-linejoin="round"
      />
      <path
         d="M4.66666 7.33325V4.66658C4.66666 3.78253 5.01785 2.93468 5.64297 2.30956C6.26809 1.68444 7.11593 1.33325 7.99999 1.33325C8.88404 1.33325 9.73189 1.68444 10.357 2.30956C10.9821 2.93468 11.3333 3.78253 11.3333 4.66658V7.33325"
         stroke="#315BFF"
         stroke-width="2"
         stroke-linecap="round"
         stroke-linejoin="round"
      />
   </svg>
);

export const LeftArrowCircleOutlinedIcon = () => (
   <svg
      width="40"
      height="40"
      viewBox="0 0 40 40"
      fill="none"
      xmlns="http://www.w3.org/2000/svg"
   >
      <path
         d="M20 36.6666C29.2048 36.6666 36.6667 29.2047 36.6667 19.9999C36.6667 10.7952 29.2048 3.33325 20 3.33325C10.7953 3.33325 3.33334 10.7952 3.33334 19.9999C3.33334 29.2047 10.7953 36.6666 20 36.6666Z"
         stroke="#C1C0C0"
         stroke-width="2"
         stroke-linecap="round"
         stroke-linejoin="round"
      />
      <path
         d="M20 13.3333L13.3333 19.9999L20 26.6666"
         stroke="#C1C0C0"
         stroke-width="2"
         stroke-linecap="round"
         stroke-linejoin="round"
      />
      <path
         d="M26.6667 20H13.3333"
         stroke="#C1C0C0"
         stroke-width="2"
         stroke-linecap="round"
         stroke-linejoin="round"
      />
   </svg>
);

export const RightArrowCircleOutlinedIcon = () => (
   <svg
      width="40"
      height="40"
      viewBox="0 0 40 40"
      fill="none"
      xmlns="http://www.w3.org/2000/svg"
   >
      <path
         d="M20 36.6666C10.7953 36.6666 3.33334 29.2047 3.33334 19.9999C3.33334 10.7952 10.7953 3.33325 20 3.33325C29.2048 3.33325 36.6667 10.7952 36.6667 19.9999C36.6667 29.2047 29.2048 36.6666 20 36.6666Z"
         stroke="#315BFF"
         stroke-width="2"
         stroke-linecap="round"
         stroke-linejoin="round"
      />
      <path
         d="M20 13.3333L26.6667 19.9999L20 26.6666"
         stroke="#315BFF"
         stroke-width="2"
         stroke-linecap="round"
         stroke-linejoin="round"
      />
      <path
         d="M13.3333 20H26.6667"
         stroke="#315BFF"
         stroke-width="2"
         stroke-linecap="round"
         stroke-linejoin="round"
      />
   </svg>
);

export const RightArrow = () => (
   <svg
      width="8"
      height="14"
      viewBox="0 0 8 14"
      fill="none"
      xmlns="http://www.w3.org/2000/svg"
   >
      <path
         d="M1 13L7 7L1 1"
         stroke="#FF3C78"
         stroke-width="2"
         stroke-linecap="round"
         stroke-linejoin="round"
      />
   </svg>
);

export const StarFilled = ({ color }) => (
   <svg
      width="16"
      height="16"
      viewBox="0 0 16 16"
      fill="none"
      xmlns="http://www.w3.org/2000/svg"
   >
      <path
         d="M6.88228 0.927054C7.18163 0.00574303 8.48504 0.0057404 8.78439 0.927051L9.9288 4.44917C10.0627 4.8612 10.4466 5.14016 10.8799 5.14016H14.5832C15.552 5.14016 15.9547 6.37977 15.171 6.94917L12.1749 9.12597C11.8244 9.38061 11.6778 9.83198 11.8117 10.244L12.9561 13.7661C13.2554 14.6874 12.2009 15.4536 11.4172 14.8842L8.42112 12.7074C8.07063 12.4527 7.59604 12.4527 7.24555 12.7074L4.24945 14.8842C3.46574 15.4536 2.41126 14.6874 2.71061 13.7661L3.85502 10.244C3.98889 9.83198 3.84223 9.38061 3.49174 9.12597L0.49565 6.94918C-0.288064 6.37978 0.114709 5.14016 1.08343 5.14016H4.78681C5.22004 5.14016 5.604 4.8612 5.73787 4.44917L6.88228 0.927054Z"
         fill={color || "#fe7831"}
      />
   </svg>
);

export const Car = () => (
   <svg
      width="36"
      height="36"
      viewBox="0 0 36 36"
      fill="none"
      xmlns="http://www.w3.org/2000/svg"
   >
      <g clip-path="url(#clip0_2_15755)">
         <path
            d="M32.8801 13.125L29.1301 15"
            stroke="#315BFF"
            stroke-width="2"
            stroke-linecap="round"
            stroke-linejoin="round"
         />
         <path
            d="M6.86993 15L3.11993 13.125"
            stroke="#315BFF"
            stroke-width="2"
            stroke-linecap="round"
            stroke-linejoin="round"
         />
         <path
            d="M27.18 20.8799H23.46"
            stroke="#315BFF"
            stroke-width="2"
            stroke-linecap="round"
            stroke-linejoin="round"
         />
         <path
            d="M12.4201 20.8799H8.70007"
            stroke="#315BFF"
            stroke-width="2"
            stroke-linecap="round"
            stroke-linejoin="round"
         />
         <path
            d="M32.8845 26.031V18.75C32.8845 16.6785 31.206 15 29.1345 15H6.86554C4.79404 15 3.11554 16.6785 3.11554 18.75V25.6365"
            stroke="#315BFF"
            stroke-width="2"
            stroke-linecap="round"
            stroke-linejoin="round"
         />
         <path
            d="M32.8846 25.6531V28.3276C32.8846 29.3551 32.0521 30.1876 31.0246 30.1876H27.3031C26.2756 30.1876 25.4431 29.3551 25.4431 28.3276V26.4676"
            stroke="#315BFF"
            stroke-width="2"
            stroke-linecap="round"
            stroke-linejoin="round"
         />
         <path
            d="M10.557 26.4661V28.3261C10.557 29.3536 9.72454 30.1861 8.69704 30.1861H4.97554C3.94804 30.1861 3.11554 29.3536 3.11554 28.3261V25.6516"
            stroke="#315BFF"
            stroke-width="2"
            stroke-linecap="round"
            stroke-linejoin="round"
         />
         <path
            d="M25.4401 26.4602H10.5601"
            stroke="#315BFF"
            stroke-width="2"
            stroke-linecap="round"
            stroke-linejoin="round"
         />
         <path
            d="M29.136 14.9986L27.81 8.66413C27.447 6.92713 25.914 5.68213 24.1395 5.68213H12.0195C10.278 5.68213 8.7645 6.88213 8.3685 8.57863L6.867 15.0001"
            stroke="#315BFF"
            stroke-width="2"
            stroke-linecap="round"
            stroke-linejoin="round"
         />
      </g>
      <defs>
         <clipPath id="clip0_2_15755">
            <rect width="36" height="36" fill="white" />
         </clipPath>
      </defs>
   </svg>
);

export const Wallet = () => (
   <svg
      width="36"
      height="36"
      viewBox="0 0 36 36"
      fill="none"
      xmlns="http://www.w3.org/2000/svg"
   >
      <path
         d="M30 27.75C30 29.4075 28.6575 30.75 27 30.75H7.5C5.8425 30.75 4.5 29.4075 4.5 27.75V7.5"
         stroke="#315BFF"
         stroke-width="2"
         stroke-linecap="round"
         stroke-linejoin="round"
      />
      <path
         d="M25.5 5.25H6.75C5.508 5.25 4.5 6.258 4.5 7.5C4.5 8.742 5.508 9.75 6.75 9.75H27C28.6575 9.75 30 11.0925 30 12.75"
         stroke="#315BFF"
         stroke-width="2"
         stroke-linecap="round"
         stroke-linejoin="round"
      />
      <path
         d="M30 12.75V17.25"
         stroke="#315BFF"
         stroke-width="2"
         stroke-linecap="round"
         stroke-linejoin="round"
      />
      <path
         d="M30 23.25V27.75"
         stroke="#315BFF"
         stroke-width="2"
         stroke-linecap="round"
         stroke-linejoin="round"
      />
      <path
         d="M30 23.25H27C25.3425 23.25 24 21.9075 24 20.25C24 18.5925 25.3425 17.25 27 17.25H30C30.828 17.25 31.5 17.922 31.5 18.75V21.75C31.5 22.578 30.828 23.25 30 23.25Z"
         stroke="#315BFF"
         stroke-width="2"
         stroke-linecap="round"
         stroke-linejoin="round"
      />
   </svg>
);

export const HeadPhone = () => (
   <svg
      width="30"
      height="30"
      viewBox="0 0 30 30"
      fill="none"
      xmlns="http://www.w3.org/2000/svg"
   >
      <path
         d="M14.9999 1.5C7.5719 1.5 1.4939 7.578 1.4939 15.006V19.8855"
         stroke="#315BFF"
         stroke-width="2"
         stroke-linecap="round"
         stroke-linejoin="round"
      />
      <path
         d="M14.9999 1.5C22.4279 1.5 28.5059 7.578 28.5059 15.006V19.8855"
         stroke="#315BFF"
         stroke-width="2"
         stroke-linecap="round"
         stroke-linejoin="round"
      />
      <path
         d="M23.4914 28.4999H25.1624C27.0089 28.4999 28.5059 27.0029 28.5059 25.1564V18.4709H23.4914C22.5689 18.4709 21.8204 19.2194 21.8204 20.1419V26.8274C21.8204 27.7514 22.5674 28.4999 23.4914 28.4999Z"
         stroke="#315BFF"
         stroke-width="2"
         stroke-linecap="round"
         stroke-linejoin="round"
      />
      <path
         d="M6.5084 28.4999H4.8374C2.9909 28.4999 1.4939 27.0029 1.4939 25.1564V18.4709H6.5084C7.4309 18.4709 8.1794 19.2194 8.1794 20.1419V26.8274C8.1794 27.7514 7.4324 28.4999 6.5084 28.4999Z"
         stroke="#315BFF"
         stroke-width="2"
         stroke-linecap="round"
         stroke-linejoin="round"
      />
   </svg>
);

export const Quality = () => (
   <svg
      width="36"
      height="36"
      viewBox="0 0 36 36"
      fill="none"
      xmlns="http://www.w3.org/2000/svg"
   >
      <path
         d="M31.5 24.0526V11.9476C31.5 10.8706 30.9255 9.87464 29.9925 9.33614L19.509 3.28364C18.576 2.74514 17.4255 2.74514 16.4925 3.28364L6.0075 9.33464C5.0745 9.87464 4.5 10.8691 4.5 11.9476V24.0526C4.5 25.1296 5.0745 26.1256 6.0075 26.6641L16.491 32.7166C17.424 33.2551 18.5745 33.2551 19.5075 32.7166L29.991 26.6641C30.9255 26.1256 31.5 25.1311 31.5 24.0526Z"
         stroke="#315BFF"
         stroke-width="2"
         stroke-linecap="round"
         stroke-linejoin="round"
      />
      <path
         d="M4.92001 25.5451L31.08 10.4551"
         stroke="#315BFF"
         stroke-width="2"
         stroke-linecap="round"
         stroke-linejoin="round"
      />
      <path
         d="M18 2.92505V33.075"
         stroke="#315BFF"
         stroke-width="2"
         stroke-linecap="round"
         stroke-linejoin="round"
      />
      <path
         d="M31.08 25.5451L4.92001 10.4551"
         stroke="#315BFF"
         stroke-width="2"
         stroke-linecap="round"
         stroke-linejoin="round"
      />
   </svg>
);

export const Play = () => (
   <svg
      width="11"
      height="14"
      viewBox="0 0 11 14"
      fill="none"
      xmlns="http://www.w3.org/2000/svg"
   >
      <path
         d="M10.5 6.13398C11.1667 6.51888 11.1667 7.48112 10.5 7.86603L1.5 13.0622C0.833333 13.4471 0 12.966 0 12.1962L0 1.80385C0 1.03405 0.833333 0.552922 1.5 0.937822L10.5 6.13398Z"
         fill="white"
      />
   </svg>
);

export const Twitter = () => (
   <svg
      width="16"
      height="13"
      viewBox="0 0 16 13"
      fill="none"
      xmlns="http://www.w3.org/2000/svg"
   >
      <path
         d="M16 1.5C15.4 1.8 14.8 1.9 14.1 2C14.8 1.6 15.3 1 15.5 0.2C14.9 0.6 14.2 0.8 13.4 1C12.8 0.4 11.9 0 11 0C9.3 0 7.8 1.5 7.8 3.3C7.8 3.6 7.8 3.8 7.9 4C5.2 3.9 2.7 2.6 1.1 0.6C0.8 1.1 0.7 1.6 0.7 2.3C0.7 3.4 1.3 4.4 2.2 5C1.7 5 1.2 4.8 0.7 4.6C0.7 6.2 1.8 7.5 3.3 7.8C3 7.9 2.7 7.9 2.4 7.9C2.2 7.9 2 7.9 1.8 7.8C2.2 9.1 3.4 10.1 4.9 10.1C3.8 11 2.4 11.5 0.8 11.5C0.5 11.5 0.3 11.5 0 11.5C1.5 12.4 3.2 13 5 13C11 13 14.3 8 14.3 3.7C14.3 3.6 14.3 3.4 14.3 3.3C15 2.8 15.6 2.2 16 1.5Z"
         fill="#FF3C78"
      />
   </svg>
);

export const Facebook = () => (
   <svg
      width="16"
      height="16"
      viewBox="0 0 16 16"
      fill="none"
      xmlns="http://www.w3.org/2000/svg"
   >
      <path
         d="M16 8.04813C16.0003 6.51922 15.5625 5.02223 14.7385 3.73441C13.9144 2.44659 12.7385 1.42189 11.3501 0.781636C9.96173 0.14138 8.41893 -0.0876158 6.90442 0.121765C5.38991 0.331145 3.96712 0.970131 2.80451 1.96306C1.6419 2.956 0.788165 4.26129 0.344396 5.72438C-0.0993733 7.18748 -0.114591 8.7471 0.300544 10.2186C0.71568 11.6901 1.54378 13.0118 2.68679 14.0272C3.82981 15.0426 5.23986 15.7092 6.75 15.9481V10.3601H4.719V8.04813H6.75V6.28513C6.70627 5.87358 6.7536 5.45745 6.88863 5.06623C7.02366 4.675 7.24309 4.31827 7.53136 4.02131C7.81964 3.72436 8.1697 3.49444 8.55674 3.34786C8.94379 3.20128 9.35833 3.14163 9.771 3.17313C10.371 3.18101 10.9696 3.23316 11.562 3.32913V5.30013H10.554C10.3823 5.27768 10.2077 5.29412 10.0432 5.34825C9.87874 5.40237 9.72853 5.49279 9.6037 5.61282C9.47887 5.73284 9.38263 5.8794 9.3221 6.04165C9.26157 6.20389 9.23829 6.37768 9.254 6.55013V8.05013H11.473L11.118 10.3621H9.25V15.9531C11.1322 15.6554 12.8464 14.6955 14.0838 13.2463C15.3213 11.7971 16.0008 9.95377 16 8.04813Z"
         fill="#FF3C78"
      />
   </svg>
);

export const Instagram = () => (
   <svg
      width="16"
      height="16"
      viewBox="0 0 16 16"
      fill="none"
      xmlns="http://www.w3.org/2000/svg"
   >
      <path
         d="M12.145 4.85213C12.6752 4.85213 13.105 4.42232 13.105 3.89213C13.105 3.36194 12.6752 2.93213 12.145 2.93213C11.6148 2.93213 11.185 3.36194 11.185 3.89213C11.185 4.42232 11.6148 4.85213 12.145 4.85213Z"
         fill="#FF3C78"
      />
      <path
         d="M8 12C5.794 12 4 10.206 4 8C4 5.794 5.794 4 8 4C10.206 4 12 5.794 12 8C12 10.206 10.206 12 8 12ZM8 6C6.897 6 6 6.897 6 8C6 9.103 6.897 10 8 10C9.103 10 10 9.103 10 8C10 6.897 9.103 6 8 6Z"
         fill="#FF3C78"
      />
      <path
         d="M12 16H4C1.944 16 0 14.056 0 12V4C0 1.944 1.944 0 4 0H12C14.056 0 16 1.944 16 4V12C16 14.056 14.056 16 12 16ZM4 2C3.065 2 2 3.065 2 4V12C2 12.953 3.047 14 4 14H12C12.935 14 14 12.935 14 12V4C14 3.065 12.935 2 12 2H4Z"
         fill="#FF3C78"
      />
   </svg>
);
